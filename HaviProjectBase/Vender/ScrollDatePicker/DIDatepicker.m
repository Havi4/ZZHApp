...
- (void)updateDatesView
{
    ...
    for (NSDate *date in self.dates) {
        DIDatepickerDateView *dateView = [[DIDatepickerDateView alloc] initWithFrame:CGRectMake(currentItemXPosition, 0, kScreen_Width, self.frame.size.height)];
        dateView.date = date;
        ...
        currentItemXPosition += kScreen_Width + kDIDatepickerSpaceBetweenItems;
    }
...
- (void)updateSelectedDatePosition
{
    ...
    CGSize itemSize = CGSizeMake(kScreen_Width + kDIDatepickerSpaceBetweenItems, self.frame.size.height);
    CGFloat itemOffset = itemSize.width * itemIndex - (self.frame.size.width - (kScreen_Width + 2 * kDIDatepickerSpaceBetweenItems)) / 2;
...
- (void)scrollViewStopAnimation:(UIScrollView *)scrollView
{
    ...
    float dateWidth = kScreen_Width + 10;
