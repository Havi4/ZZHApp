...
- (void)creatBackView:(UIImageView *)backView
{
    ...
    for (int i = 0; i<7; i++) {
        ...
        UIImageView *viewBack = [[UIImageView alloc]initWithFrame:CGRectMake(currentItemXPosition, (self.frame.size.height - 20)*0.35/2 + (dateHeight - kScreen_Width)/2, kScreen_Width, kScreen_Width)];
        viewBack.layer.cornerRadius = (kScreen_Width)/2;
...
//            viewBack.backgroundColor = [UIColor colorWithRed:0.969f green:0.463f blue:0.388f alpha:1.00f];
        ...
        currentItemXPosition += kScreen_Width + 10;
...
- (void)creatMonthButton
{
    ...
    [_calenderButton makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(_monthLabel.mas_centerY);
        make.left.equalTo(_monthLabel.mas_right).offset(5);
    }];
    self.backLine = [[UIImageView alloc]init];
    self.backLine.dk_backgroundColorPicker = kTextColorPicker;
    [self addSubview:self.backLine];
    [_backLine makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@1);
        make.top.equalTo(self.monthLabel.mas_bottom).offset(0);
        make.left.equalTo(self.monthLabel.mas_left);
        make.right.equalTo(_calenderButton.mas_right).offset(0);
    }];
...
- (void)creatTapImage
{
    CGFloat currentItemXPosition = 10;
    for (int i = 0; i<7; i++) {
        UIView *viewBack = [[UIView alloc]initWithFrame:CGRectMake(currentItemXPosition, (self.frame.size.height-35)*0.65+35, kScreen_Width, (self.frame.size.height-35)*0.35)];
