...
#define DAY_TITLE_FONT_SIZE 20.f
...
#define DATE_LABEL_INFO_WIDTH 112.f
...
static NSString* const CLCalendarSelectedDatePrintFormatDefault = @"yyyy-MM-dd HH:mm:ss";
...
@property (nonatomic, strong) LabelLine *dateInfoLabel;
...
-(UIView *)dailyInfoSubViewContainer
{
    if(!_dailyInfoSubViewContainer){
        _dailyInfoSubViewContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0 , self.bounds.size.width, DATE_LABEL_INFO_HEIGHT)];
        ...
        [_dailyInfoSubViewContainer addSubview:self.calendarView];
    }
...
{
    if(!_dateInfoLabel){
        _dateInfoLabel = [[LabelLine alloc] initWithFrame:CGRectMake(WEATHER_ICON_LEFT+WEATHER_ICON_WIDTH+DATE_LABEL_MARGIN_LEFT, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT)];
        ...
        [_dateInfoLabel addGestureRecognizer:singleFingerTap];
    }
    _dateInfoLabel.font = [UIFont systemFontOfSize: self.selectedDatePrintFontSize];
    _dateInfoLabel.dk_textColorPicker = kTextColorPicker;
    return _dateInfoLabel;
}
-(UIView *)dayTitleSubViewContainer
{
    if(!_dayTitleSubViewContainer){
        _dayTitleSubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, DATE_LABEL_INFO_HEIGHT+1.5*DATE_VIEW_MARGIN, self.bounds.size.width, DAY_TITLE_VIEW_HEIGHT)];
...
-(UIView *)dailySubViewContainer
{
    if(!_dailySubViewContainer){
        _dailySubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, DAY_TITLE_VIEW_HEIGHT+DATE_LABEL_INFO_HEIGHT+2.5*DATE_VIEW_MARGIN, self.bounds.size.width, DATE_VIEW_HEIGHT)];
...
-(UIView *)backgroundImageView
{
    if(!_backgroundImageView){
        _backgroundImageView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
-(UILabel *)dayTitleViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    ...
    dayTitleLabel.dk_textColorPicker = kTextColorPicker;
...
-(void)markDateSelected:(NSDate *)date
{
    ...
    NSString *day = [NSString stringWithFormat:@"%@年%@月",[strDate substringWithRange:NSMakeRange(0, 4)],[strDate substringWithRange:NSMakeRange(5, 2)]];
