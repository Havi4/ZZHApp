...
-(UILabel *)dateLabel
{
    if(!_dateLabel){
        ...
        _dateLabel.dk_textColorPicker = kTextColorPicker;
...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        ...
        self.dateLabelContainer.dk_backgroundColorPicker = (blnSelected)?DKColorWithColors(kLightColor, kLightColor): DKColorWithColors([UIColor clearColor], [UIColor clearColor]);
        
        self.dateLabel.dk_textColorPicker = (blnSelected)?DKColorWithColors(kDefaultColor, kDefaultColor):kTextColorPicker;
//        textColor = (blnSelected)?[UIColor colorWithRed:52.0/255.0 green:161.0/255.0 blue:255.0/255.0 alpha:1.0]:[self colorByDate];
    }
