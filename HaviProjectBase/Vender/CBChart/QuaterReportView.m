...
@implementation QuaterReportView
- (instancetype)init
{
    self = [super init];
    if (self) {
...
+(instancetype)QuaterReportView
{
    QuaterReportView *chartView = [[self alloc] init];
    // 默认值
...
//        PNBar * bar = [[PNBar alloc] initWithFrame:CGRectMake(xPoint.x+10, 20+(yCoordinateHeight-15)/5*(5-gradePercent), (xCoordinateWidth-15-6)/_funcPoints.count-20, (yCoordinateHeight-15)/5*gradePercent)];
    ...
    for (int i=0; i<_funcPoints.count; i++) {
        ...
        PNBar * bar = [[PNBar alloc] initWithFrame:CGRectMake(padding*i + self.leftLineMargin+5, 20+(yCoordinateHeight-15)/5*(5-gradePercent), (xCoordinateWidth-15-6)/_funcPoints.count, (yCoordinateHeight-15)/5*gradePercent)];
        bar.grade = 1;
        [self addSubview:bar];
    }
     */
