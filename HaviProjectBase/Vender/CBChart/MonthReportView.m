...
{
    MonthReportView *chartView = [[self alloc] init];
    // 默认值
...
- (void)setYCoorLabel
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, -25, 60, 30)];
...
- (void)drawFuncLine
{
    ...
    int padding = (xCoordinateWidth-15-6)/_funcPoints.count;
    for (int i=0; i<_funcPoints.count; i++) {
        int gradePercent = [[_funcPoints objectAtIndex:i]intValue];
        PNBar * bar = [[PNBar alloc] initWithFrame:CGRectMake(padding*i + self.leftLineMargin+5+1, 20+(yCoordinateHeight-15)/5*(5-gradePercent), (xCoordinateWidth-15-6)/_funcPoints.count-2, (yCoordinateHeight-15)/5*gradePercent)];
...
-(void)setUpXcoorWithValues:(NSArray *)values
{
    ...
    if (values.count){
        ...
        for (int i = 0; i < count; i++) {
            ...
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(cX - size.width * 0.5, cY + 5, ((xCoordinateWidth-15)/(count-1)), 10)];
            label.text = xValue;
            label.tag = 1000;
            label.textColor = selectedThemeIndex==0?kDefaultColor:[UIColor whiteColor];
