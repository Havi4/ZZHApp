...
-(void)setupPagingProcess{
    ...
    if(self.needToShowPageControl){
        ...
        if (!self.pageControl) {
            self.pageControl               = [[HAVIPageControl alloc] init];
       ...
       && self.viewControllers.count > 0){
        ...
        [sortedIndexes enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if([self useAutoLayout:v]){
                ...
                v.frame = (CGRect){SCREEN_SIZE.width * i, 64, SCREEN_SIZE.width, CGRectGetHeight(self.view.frame)-64};
                i++;
            }
        }];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [self scrollTitle:scrollView subviews:self.navItemsViews];
    
}
//add by havi
- (void)scrollTitle:(UIScrollView *)scroll subviews:(NSArray *)subviews
{
